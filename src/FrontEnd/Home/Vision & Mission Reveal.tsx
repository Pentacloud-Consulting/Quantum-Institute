          ...
          <div className="absolute inset-0 flex items-center w-full pointer-events-none">
            <AnimatePresence mode="wait">
              {step === 1 && (
                ...
                >
                    ...
                    <div className="flex flex-col">
                      {[
                        ...
                        <div key={idx} className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4 py-3 border-b border-gray-100 last:border-0 group">
                          <span className="text-[11px] font-bold uppercase tracking-widest text-black w-32 shrink-0 pt-1 group-hover:text-[#d15000] transition-colors">{item.title}</span>
                          <div className="flex flex-wrap gap-1.5">
                            {item.tags.map((tag, i) => (
                              <span key={i} className="px-3 py-1 bg-gray-50/80 text-gray-600 text-[11px] font-medium rounded-full border border-gray-200/80 hover:border-[#d15000]/40 hover:bg-[#d15000]/5 hover:text-black transition-all cursor-default">
                                {tag}
                              </span>
                            ))}
                          </div>
                  ...
                  <div className="flex flex-wrap items-center gap-8 mt-6">
                    <button className="group flex items-center gap-4 px-8 py-3 border border-gray-300 rounded-full bg-transparent text-black text-[10px] font-bold tracking-[0.2em] uppercase hover:border-[#d15000] hover:bg-[#d15000]/5 transition-colors duration-300">
                      Explore Our Vision
              ...
              {step === 2 && (
                ...
                >
                  <div className="flex flex-col items-start">
                    <h4 className="text-[#d15000] text-xs font-bold tracking-[0.3em] uppercase mb-3">Mission</h4>
                    <div className="w-8 h-[2px] bg-[#d15000]"></div>
                  </div>
                  ...
                  <div className="flex flex-col mt-10 w-full max-w-lg">
                    ...
                    <div className="flex flex-col">
                      {[
                        ...
                        <div key={idx} className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4 py-3 border-b border-gray-100 last:border-0 group">
                          <span className="text-[11px] font-bold uppercase tracking-widest text-black w-40 shrink-0 pt-1 group-hover:text-[#d15000] transition-colors">{item.title}</span>
                          <div className="flex flex-wrap gap-1.5">
                            {item.tags.map((tag, i) => (
                              <span key={i} className="px-3 py-1 bg-gray-50/80 text-gray-600 text-[11px] font-medium rounded-full border border-gray-200/80 hover:border-[#d15000]/40 hover:bg-[#d15000]/5 hover:text-black transition-all cursor-default">
                                {tag}
